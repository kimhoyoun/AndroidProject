...
//                polyline1.setColor(COLOR_BLACK_ARGB);
                ...
                mDatabaseRef.child("UserAccount").child(firebaseUser.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        dlg.setMessage("거리는 "+ getDistance(lastx, lasty, firstx, firsty) + "m 이고," + "\n" + "소모한 칼로리는 "
...
//                                mDatabaseRef.child("Record").child("Sequence").setValue(seq +1);
    ...
    public double kcal(long endTime, long startTime, String weight){
        ...
        System.out.println("KcalTime : "+KcalTime + "여기 있어요오오오오오오오오오오");
        System.out.println("restTime : "+restTime + "여기 있어요오오오오오오오오오오");
        ...
        if(choice == 1) {
            runmets = 12.25 * userWeight * (KcalTime-restTime); // 4초면 2450.0
        } else {
            runmets = 14 * userWeight * (KcalTime-restTime);
        }

        double onekcal = runmets * 0.005; // 1칼로리는 12.25
        double totalkcal = onekcal * (KcalTime-restTime); // 4초뛰면 49 칼로리
        System.out.println("totalKcal " + totalkcal + "여기 있어요오오오오오오오오오");
